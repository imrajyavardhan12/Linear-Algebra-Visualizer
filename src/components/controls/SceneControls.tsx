import { EXAMPLE_LABELS, type ExampleName } from '../../state/examples';
...
interface SceneControlsProps {
  ...
  onReset: () => void;
  onLoadExample: (example: ExampleName) => void;
  theme: 'dark' | 'light';
  onToggleTheme: () => void;
}

export function SceneControls({ showStandardBasis, onToggleStandardBasis, onReset, onLoadExample, theme, onToggleTheme }: SceneControlsProps) {
  return <section className="scene-options" aria-label="Scene options">
    ...
    <div className="example-controls">
      <span className="block-label">Quick scenes</span>
      <div className="example-buttons">
        {(Object.keys(EXAMPLE_LABELS) as ExampleName[]).map((example) => <button className="example-button" type="button" key={example} onClick={() => onLoadExample(example)}>{EXAMPLE_LABELS[example]}</button>)}
      </div>
    </div>
  </section>;
}
