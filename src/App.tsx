...
import { analyzeVectorSet, evaluateLinearCombination, formatNumber } from './math';
...
function App() {
  ...
  const combinationEvaluation = useMemo(() => {
    ...
    return first && second ? evaluateLinearCombination(first, second, state.coefficients) : null;
    ...
    <main className="app-main">
      ...
      <div className="workspace-grid">
        <section className="visualization-column" aria-label="Vector visualization">
          <VectorPlane vectors={planeVectors} analysis={analysis} onChange={actions.setVector} showStandardBasis={state.showStandardBasis} combination={combinationEvaluation ? { enabled: state.showCombination, evaluation: combinationEvaluation } : undefined} />
        ...
        <aside className="control-rail" aria-label="Vector controls and mathematical explanation">
          ...
          <CoefficientControls coefficients={state.coefficients} evaluation={combinationEvaluation} enabled={state.showCombination} onToggle={actions.toggleCombination} onChange={actions.setCoefficient} />
